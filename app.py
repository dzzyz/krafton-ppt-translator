...
EN_FONT = "Calibri"
ADMIN_PASSWORD = "krafton2026"  # ← 나중에 바꾸세요!
...
def detect_slide_type(texts):
    ...
    if any(k in combined for k in ["승인사항", "승인 요청", "의결"]):   return "approval"
    if any(k in combined for k in ["억원", "매출", "손익", "YoY"]):     return "financial"
    if any(k in combined for k in ["출시", "파이프라인", "Q1", "Q2"]):  return "timeline"
    if "사전공유" in combined:                                           return "pre-disclosure"
    return "strategy"
...
Rules:
1. Return ONLY valid JSON, same keys, string values
2. Preserve \\n line breaks
...
Output:"""
    ...
    res = client.messages.create(
        ...
        system="Precise JSON-output PPT translator for KRAFTON BOD. Return ONLY valid JSON.",
...
def replace_para_text(para, new_text, shape=None, min_pt=7):
    ...
    # ── 폰트 축소 제외 조건 ────────────────────────────────
    # 아래 조건 중 하나라도 해당되면 원본 폰트 크기 그대로 유지
    text_stripped = new_text.strip()
    is_title      = base_pt >= 18                          # 타이틀/헤드라인
    is_short      = len(text_stripped.replace("\n","")) <= 30  # DRI, 페이지번호 등 짧은 텍스트
    is_label_only = bool(re.match(r'^[\d\s\.\)\-\|/A-Za-z:]+$', text_stripped))  # 숫자/영문만 (주석, 페이지번호)
...
with tab_translate:
    ...
    st.caption(f"현재 적용 Glossary: **{len(active_glossary)}개** 항목 "
               f"(기본 {len(BASE_GLOSSARY)} + 승인 {len(st.session_state.approved_glossary)} "
    ...
    if uploaded_file and api_key:
        ...
        if st.button("🚀 번역 시작", type="primary", use_container_width=True):
            ...
            st.download_button(
                ...
                data=output, file_name=out_name,
                mime="application/vnd.openxmlformats-officedocument.presentationml.presentation",
                use_container_width=True, type="primary"
            )

# ══════════════════════════════════════════════════════════
# TAB 2: Glossary 보기 + 유저 제안
# ══════════════════════════════════════════════════════════
with tab_glossary:
    st.header("📖 Glossary")
    ...
    st.caption("번역이 어색하거나 더 좋은 표현을 발견하면 제안해주세요. "
    ...
    with st.form("suggest_form", clear_on_submit=True):
        ...
        if submitted:
            if not new_ko.strip() or not new_en.strip():
                ...
                st.success(f"✅ **`{new_ko}` → {new_en}** 제안 완료! "
                           f"이번 세션 번역에 즉시 적용됩니다. 관리자 승인 후 전체 반영.")
                ...
                with st.container(border=True):
                    ...
                    with c4:
                        if st.button("✅ 승인", key=f"approve_{idx}", type="primary"):
                            ...
                            st.toast(f"✅ `{item['ko']}` 승인 완료!", icon="✅")
                            st.rerun()
                    with c5:
                        if st.button("❌ 거절", key=f"reject_{idx}"):
                            pending.pop(idx)
                            st.session_state.pending_glossary = pending
                            st.toast(f"❌ `{item['ko']}` 거절됨", icon="❌")
                            st.rerun()
